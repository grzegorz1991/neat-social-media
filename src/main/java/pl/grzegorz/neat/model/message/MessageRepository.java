...
//    Page<MessageEntity> findNonArchivedMessagesForUser(UserEntity user, Pageable pageable);
    ...
    @Query("SELECT m FROM MessageEntity m WHERE (m.sender = :user AND m.senderArchived = true) OR (m.receiver = :user AND m.recipentArchived = true) ORDER BY m.timestamp DESC")
    Page<MessageEntity> findArchivedMessagesByUser(@Param("user") UserEntity user, Pageable pageable);
    ...
    @Query("SELECT m FROM MessageEntity m WHERE m.receiver = :user AND m.recipentArchived = false ORDER BY m.timestamp DESC")
    List<MessageEntity> findAllNonArchivedMessagesByReceiver(UserEntity user);

    @Query("SELECT m FROM MessageEntity m WHERE m.sender = :user AND m.senderArchived = false ORDER BY m.timestamp DESC")
    Page<MessageEntity> findAllNonArchivedMessagesBySender(UserEntity user, Pageable pageable);
